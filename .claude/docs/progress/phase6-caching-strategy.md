...
### 1. Optimistic Updates (P1)

- 네트워크 응답 대기 없이 UI 즉시 변경 (즐겨찾기 토글, 북마크 수정 등)

### 2. 스마트 동기화 및 수동 갱신 (P1)

**배경**: Eternal 캐싱은 로컬 속도는 최강이지만, 다른 기기(PC 등)에서 발생한 데이터 변화를 감지하지 못함.

- [ ] **변경 감지용 '핑(Ping)' API 도입**
  - 앱 포커싱 시 유저별 `last_updated_at` 시간값 하나만 조회 (초경량 쿼리)
  - 로컬 캐시의 마지막 업데이트 시간보다 서버 시간이 최신일 경우에만 `invalidateQueries` 실행
- [ ] **Pull to Refresh 구현**
  - 모바일 사용자 경험(UX)을 위한 화면 당겨서 새로고침 기능 추가
  - 당기는 동작 시 "당겨서 새로고침" 문구 노출 및 애니메이션 적용
  - 강제 `invalidate` 트리거로 기기 간 데이터 불일치 수동 해결 통로 제공
